...
export async function POST(request: NextRequest) {
  try {
    ...
    const { nome, telefone_ia, tipo_marcacao, email, senha, fase, z_api_instance_id, z_api_token } = body;
    ...
    try {
      ...
      if (existingInstance) {
        ...
        if (z_api_instance_id) insertData.z_api_instance_id = z_api_instance_id;
        if (z_api_token) insertData.z_api_token = z_api_token;
        ...
        if (instanceError) {
          ...
          // Instâncias Z-API são pré-criadas no painel Z-API; não há criação programática
        }
      }
